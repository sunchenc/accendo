...
function put_hcfa($line, $col, $maxlen, $data,$pdf) {
    ...
    //commented by Qiao,oupper characters is unecessary
...
function gen_hcfa_1500($options, $encounter_claim=false) {
...
function gen_hcfa_1500_page(&$claim) {
    ...
    put_hcfa(2, 48, 31, $claim->payerName());
    put_hcfa(3, 48, 31, $claim->payerStreet());
    // Payer city, state, zip.
    $tmp = $claim->payerCity() ? ($claim->payerCity() . ', ') : '';
    put_hcfa(4, 48, 31, $tmp . $claim->payerState() . ' ' . $claim->payerZip());
    ...
    /*
      ...
      $tmpcol = 24; // Champus VA (why same code?)
      else if ($ct === 'BL')
    ...
    put_hcfa(8, 62, 17, $claim->policyNumber());
    ...
    put_hcfa(10, 39, 2, substr($tmp, 5, 2));
    put_hcfa(10, 42, 2, substr($tmp, 8, 2));
    put_hcfa(10, 45, 4, substr($tmp, 0, 4));
    put_hcfa(10, $claim->patientSex() == 'M' ? 50 : 56, 1, 'X');
 ...
 put_hcfa(12, $tmpcol, 1, 'X');
    ...
    put_hcfa(14, 33, 2, $claim->patientState());
...
//    put_hcfa(14, 74, 2, $claim->insuredState());
    ...
    put_hcfa(16, 19, 3, substr($tmp, 0, 3));
    put_hcfa(16, 24, 7, substr($tmp, 3));
...
//    }
    ...
    if (!empty($diags[0])) {
        ...
        put_hcfa(38, 5, 6, $diags[0]);
        //     put_hcfa(38, 7, 2, substr($diags[0], 3));
    }
    if (!empty($diags[2])) {
        put_hcfa(38, 32, 3, substr($diags[2], 0, 3));
...
//    put_hcfa(38, 62, 10, $claim->medicaidOriginalReference());
    ...
    if (!empty($diags[1])) {
        put_hcfa(40, 5, 3, substr($diags[1], 0, 3));
        //   put_hcfa(40, 7, 2, substr($diags[1], 3));
    }
    if (!empty($diags[3])) {
        put_hcfa(40, 32, 3, substr($diags[3], 0, 3));
...
//        put_hcfa($lino, 16, 2, substr($tmp, 2, 2));
        ...
        put_hcfa($lino, 22, 2, $claim->cpt_place_of_service($prockey));
        //
        // 24c. EMG
        // Not currently supported.
        ...
        $arr = explode(':', $tmp);
        if (count($arr) > 0) {
            put_hcfa($lino, 30, 7, $arr[0]);
            for ($i = 1; $i < count($arr); $i++) {
                $colo = 37 + $i * 2;
                put_hcfa($lino, $colo, 2, $arr[$i]);
            }
        }

        //    put_hcfa($lino, 39, 6, $claim->cptModifier($hcfa_proc_index));
        // 24e. Diagnosis Pointer
        $tmp = str_replace(':', ',', $claim->diagnosis_pointer($prockey));
        put_hcfa($lino, 53, 4, $tmp);

        // 24f. Charges
        put_hcfa($lino, 61, 8, str_replace('.', ' ',
                        sprintf('%.2f', $claim->cptCharges($prockey))));
        // 24g. Days or Units
        put_hcfa($lino, 69, 3, $claim->cpt_days_or_units($prockey));

        // 24h. EPSDT Family Plan
        // Not currently supported.
        // 24j. Rendering Provider NPI
        put_hcfa($lino, 80, 10, $claim->renderingprovider_NPI());
    }
...
//    }
    ...
    put_hcfa(56, 1, 15, $claim->provider_Tax_id_number());
//    if ($claim->federalIdType() == 'SY') {
//        put_hcfa(56, 16, 1, 'X'); // The SSN checkbox
//    } else {
    put_hcfa(56, 25, 1, 'X'); // The EIN checkbox
//    }
    // 26. Patient's Account No.
    // Instructions say hyphens are not allowed, but freeb used them.
    put_hcfa(56, 29, 15, $claim->patient_account());

    // 27. Accept Assignment
    put_hcfa(56, $claim->billingFacilityAssignment() ? 47 : 53, 1, 'X');

    // 28. Total Charge
    put_hcfa(56, 66, 8, str_replace('.', ' ', sprintf('%.2f', $claim->total_charge())));

    // 29. Amount Paid
//    put_hcfa(56, 62, 8, str_replace('.', ' ', sprintf('%8.2f', $claim->expected_payment())));
//    // 30. Balance Due
...
//                            $clm_total_charges - $clm_amount_paid - $clm_amount_adjusted)));
    ...
    put_hcfa(57, 85, 3, substr($tmp, 0, 3));
    put_hcfa(57, 89, 7, substr($tmp, 3));

    // 32. Service Facility Location Information: Name
    put_hcfa(58, 30, 30, $claim->facilityName());

    // 33. Billing Provider: Name
    put_hcfa(58, 63, 25, $claim->billing_provider_name());

    // 32. Service Facility Location Information: Street
    put_hcfa(59, 30, 25, $claim->facilityStreet());

    // 33. Billing Provider: Name
    put_hcfa(59, 63, 25, $claim->billing_provider_street());


    // 32. Service Facility Location Information: City State Zip
    $tmp = $claim->facilityCity() ? ($claim->facilityCity() . ' ') : '';
    put_hcfa(60, 30, 30, $tmp . $claim->facilityState() . ' ' .
            $claim->facilityZip());
    // 33. Billing Provider: City State Zip
    $tmp = $claim->billing_provider_city() ? ($claim->billing_provider_city() . ' ') : '';
    put_hcfa(60, 63, 25, $tmp . $claim->billing_provider_state() . ' ' .
    ...
    $tmp = $claim->rendering_provider_FirstName() . ' ' . $claim->rendering_provider_LastName();
    put_hcfa(61, 1, 20, $tmp);
    if ($claim->procCount() > 0) {
        $tmp = $claim->cpt_serviceDate(0);
        put_hcfa(61, 21, 2, substr($tmp, 4, 2));
        put_hcfa(61, 23, 2, substr($tmp, 6, 2));
        put_hcfa(61, 25, 2, substr($tmp, 2, 2));
    }
    //
